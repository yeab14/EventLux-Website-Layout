    ...
    <header className="bg-white shadow-md py-4 fixed w-full z-50">
      <div className="container mx-auto flex justify-between items-center">
        ...
        <div className="lg:hidden" onClick={() => setMenuOpen(!menuOpen)}>
          <div className={`w-6 h-0.5 bg-green-500 mb-1 transition-all ${menuOpen ? "rotate-45 translate-y-2" : ""}`}></div>
          <div className={`w-6 h-0.5 bg-green-500 mb-1 ${menuOpen ? "opacity-0" : ""}`}></div>
          <div className={`w-6 h-0.5 bg-green-500 ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}></div>
        </div>
        ...
        >
          {["Home", "About Us", "Services", "Packages", "Gallery", "Contact Us"].map((item) => (
            <Link
              key={item}
              to={`${item.toLowerCase().replace(/\s+/g, '')}Section`}
              smooth={true}
              duration={700}
              offset={-80} // Offset to consider fixed header height
